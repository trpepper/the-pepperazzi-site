...
function getBuildOutDir(config) {
  const outDir = config.build?.outDir || 'dist';

  return path.isAbsolute(outDir) ? outDir : path.join(config.root, outDir);
}

function getClientGalleryRedirects(config) {
  const galleryCodes = Object.keys(getClientGalleries(config)).sort((first, second) =>
    first.localeCompare(second, undefined, {
      numeric: true,
      sensitivity: 'base',
    }),
  );
  const galleryRedirects = galleryCodes.flatMap((code) => {
  ...
  return [
    '# Generated by vite.config.js from public/media/client_galleries.',
